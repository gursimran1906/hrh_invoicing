...
def check_subscription(request):
    ...
    if tenant.paid_until:
        ...
        one_month_away = current_date + relativedelta(days=7)
...
def all_clients_view(request):
    ...
    'date_joined', 'date_left', 'email', 'client_types',
...
def download_all_clients_data(request):
    ...
    clients = Client.objects.all().order_by('date_joined').prefetch_related('client_agreed_rate', 'allotted_room', 'payable_by')
...
def download_current_clients_data(request):
    ...
    fields = [
    'id', 'name', 'date_of_birth', 'address', 'contact_number',
    'date_joined', 'email', 'client_types',
    'client_agreed_rate',  
    'allotted_room',  
    'payable_by', 'respite','notes',]
    
    clients = Client.objects.filter(date_left__isnull=True).order_by('date_joined').prefetch_related('client_agreed_rate', 'allotted_room', 'payable_by')
...
def download_all_one_to_ones(request):
    ...
    one_to_ones = OneToOne.objects.all().order_by('date').prefetch_related('customer')
    ...
    for one_to_one in one_to_ones:
        data_row = [smart_str(getattr(one_to_one, field)) for field in one_to_one_fields]
        writer.writerow(data_row)
...
def download_this_months_one_to_ones(request):
    ...
    last_day = datetime.now().replace(day=1, month=datetime.now().month + 1, hour=0, minute=0, second=0, microsecond=0)

    one_to_ones = OneToOne.objects.filter(date__range=[first_day, last_day]).order_by('date').prefetch_related('customer')
...
def client_view(request, client_id):
    ...
    total_invoices = Invoice.objects.filter(client=client).aggregate(Sum('costs'))['costs__sum'] or Decimal('0.00')
    total_invoices = round(total_invoices, 2)

    # Calculate total monies in for the client
    total_monies_in = MoneyIn.objects.filter(invoices_to_allocate__client=client).aggregate(Sum('amount'))['amount__sum'] or Decimal('0.00')
    ...
    if request.user.is_invoice_department:
        ...
        return HttpResponseForbidden(f"You do not have access.")
...
def add_client(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, f"{form._meta.model.__name__} added successfully.")
        ...
        client_objects = Client.objects.filter(date_left__isnull=True).prefetch_related(
            'allotted_room', 'client_agreed_rate', 'payable_by'
        ).annotate(
            room_id=F('allotted_room__id'),
            daily_rate=F('client_agreed_rate__amount'),
            payable_by_name=F('payable_by__name'),
            room=F('room_id'),
            resident_ref=F('resident_name_number')
        ).order_by('allotted_room__id').values('room', 'name', 'address', 'contact_number', 'date_joined', 'email', 
            'client_types', 'daily_rate', 'payable_by_name',
...
def edit_local_authority(request, id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request,f'Local authority data successfully updated!')
            return redirect('add_local_authority')  
        else:
            messages.error(request, "Form submission failed. Please check the errors.")
    else:
...
def allocate_money_in(request, id):
    ...
    for inv_num in selected_invoices:
        ...
        balance_left_money_in = balance_left_money_in - invoice.costs
...
def get_attendance_data(request):
    ...
    present_data = {entry['date']: entry['count'] for entry in presents_per_day}
...
def generate_monthly_attendance_chart(request, month_year):
    ...
    end_date = month_year_date.replace(day=monthrange(month_year_date.year, month_year_date.month)[1])
    ...
    fig, ax = plt.subplots()
    ax.set_title(f"Bed Occupancy Chart for {month_year_date.strftime('%B %Y')}")
...
def clients_attendance(request):
    if request.method == 'POST':
        ...
        if (year, month) < (today.year, today.month):
            ...
            days_of_month = [date(year, month, day) for day in range(1, days_in_month + 1)]
        elif (year, month) == (today.year, today.month):
            # For the current month, limit days to today
            days_of_month = [date(today.year, today.month, day) for day in range(1, today.day+1)]
        else:
            days_of_month = []
            # For future months, return an error message
            messages.error(request, "Selected month is in the future. Please select current month or a month in past.")
        ...
        last_day_of_month = datetime(year, month, monthrange(year, month)[1]).date()
        ...
        for client in clients:
            ...
            client_data = {'id': client_id, 'name': client_name, 'attendance': {}}

            for day in days_of_month:
                attendance_entry = attendances.filter(client__id=client_id, date=day).first()
        ...
        context = {'clients': attendance_data, 'days_of_month': days_of_month, "selected_month_year": month_year}
    ...
    try:
        ...
        for key, value in request.POST.items():
                ...
                try:
                    ...
                    existing_attendance = Attendance.objects.filter(client=client, date=date_obj).first()
                    
                    if existing_attendance:
                        existing_attendance.present = present
                        existing_attendance.save()
                    else:
                        Attendance.objects.create(client=client, date=date_obj, present=present)
                        count += 1
                except ValueError as e:
                    messages.error(request, f'Attendance not marked. {key, value}. Error:{str(e)}')

        messages.success(request, f'Attendance successfully marked {count} times.') 
    except Exception as e:
...
def generate_monthly_invoices(request):
    try:
        ...
        clients_with_attendance = Client.objects.filter(attendance__date__range=(first_day_of_month, last_day_of_month)).distinct()

        for client in clients_with_attendance:
            # Calculate attendance count for the current month
            attendance_count = Attendance.objects.filter(client=client, date__range=(first_day_of_month, last_day_of_month)).count()

            # Calculate total cost based on attendance count and rate
            total_cost = attendance_count * client.client_agreed_rate.amount

            invoice = Invoice.objects.create(
                client=client,
                date=today,
                desc=client.client_agreed_rate,
                costs=total_cost,
                units=attendance_count, 
            )

        clients_with_one_to_one = Client.objects.filter(onetoone__date__range=(first_day_of_month, last_day_of_month)).distinct()
        one_to_one_rate = Rate.objects.filter(description='OneToOneHourly').first()
        
        for client in clients_with_one_to_one:
            # Calculate total hours for the current month
            total_hours = OneToOne.objects.filter(customer=client, date__range=(first_day_of_month, last_day_of_month)).aggregate(Sum('hours'))['hours__sum'] or 0
    ...
    try:
        ...
        for invoice in existing_invoices:
            if invoice.sent_to_client == False:
                try:
                    print(send_invoice_email(invoice.invoice_number,invoice.client.email, month_year_date))
...
def make_pdf_of_invoice(invoice_number, hide_client_details):
    ...
    if invoice.client.client_types == 'authorities_funded':
        html_content += f'''    <tr>
    ...
    if hide_client_details:
        ...
        html_content += f''' <p class='col'><strong>Client:</strong> {invoice.client.name} </p>'''

    html_content += f'''          
                          <p class='col'><strong>Date:</strong> {invoice.date}</p>
...
def send_invoice_email(invoice_number, recipient_email, month_year):
    try:
        ...
        pdf_content = make_pdf_of_invoice(invoice_number, True)
...
def download_invoice(request, invoice_number):
    ...
    pdf_content = make_pdf_of_invoice(invoice_number, False)
...
def get_one_to_one_service_usage_chart(request):
    ...
    data = OneToOne.objects.values('customer__name').annotate(total_hours=Sum('hours'))
...
def get_revenue_data(request):
    ...
    for month_year, current_revenue in sorted(months_revenue.items()):
        if previous_revenue is not None:
            percentage_increase = ((current_revenue - previous_revenue) / previous_revenue) * 100
    ...
    for month in next_three_months:
        ...
        if previous_revenue is not None:
            projected_revenue = previous_revenue * (1 + Decimal(str(average_percentage_increase)) / 100)
            months_revenue[month_year] = projected_revenue
            previous_revenue = projected_revenue

    data = [str(x.quantize(Decimal('0.00'))) for key, x in months_revenue.items()]
...
def download_invoice_accountants_csv(request):
    if request.method == 'POST':
        ...
        # Write header row
        writer.writerow(['Invoice Number', 'Client', 'Date', 'Description', 'Costs', 'Units', 'Settled', 'Sent to Client', 'Additional Notes', 'Timestamp'])
...
def download_monies_in_accountants_csv(request):
    if request.method == 'POST':
        ...
        writer = csv.writer(response)
        # Write header row
        writer.writerow(['ID', 'Payment Type', 'Amount', 'Balance Left', 'Description', 'Date', 'Invoices to Allocate', 'Timestamp'])
        ...
        for entry in moneyin_entries:
            writer.writerow([
                ...
                entry.desc,
                entry.date,
                ', '.join([str(invoice.invoice_number) for invoice in entry.invoices_to_allocate.all()]),
                entry.timestamp,
            ])
        return response
    else:
...
def download_creditnote_accountants_csv(request):
    ...
    if request.method == 'POST':
        ...
        creditnote_entries = CreditNote.objects.filter(timestamp__gte=date_from)
        ...
        writer = csv.writer(response)
        # Write header row
        writer.writerow(['ID', 'Amount', 'Invoice to Allocate', 'Notes', 'Timestamp'])
